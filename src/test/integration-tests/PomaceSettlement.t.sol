...
contract PomaceSettlementTest is EngineIntegrationFixture {
    ...
    function testCannotSettleBeforeFinalization() public {
        ...
        vm.warp(expiry);
        oracle.setExpiryPriceWithFinality(address(weth), address(usdc), 2300 * 1e6, false);
        vm.expectRevert(PM_PriceNotFinalized.selector);
    ...
    function testSettleETHCollatCall() public {
        ...
        vm.warp(expiry);
        oracle.setExpiryPrice(address(weth), address(usdc), 2500 * 1e6);
    ...
    function testSettleUSDCollatPut() public {
        ...
        vm.warp(expiry);
        oracle.setExpiryPrice(address(weth), address(usdc), 1500 * 1e6);
    ...
    function testBatchSettleSameCollat() public {
        ...
        ids[1] = _mintPutOption(2500 * 1e6, wethCollatProductId, 1 * UNIT);

        amounts[0] = 1 * UNIT;
        amounts[1] = 1 * UNIT;

        uint256 expiryPrice = 2600 * 1e6;
        ...
        uint256 expectedEthPayout = (2500 * 1e6 * UNIT / expiryPrice) * 1e12;

        assertEq(usdc.balanceOf(address(this)), selfUsdcBefore - 2000 * 1e6);
        assertEq(usdc.balanceOf(address(engine)), engineUsdcBefore + 2000 * 1e6);

        assertEq(weth.balanceOf(address(this)), expectedEthPayout);
        assertEq(weth.balanceOf(address(engine)), engineWethBefore - expectedEthPayout);
    }
